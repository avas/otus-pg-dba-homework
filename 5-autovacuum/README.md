...
```
Как видно, производительность несколько упала - с 629.2 транзакций в секунду в среднем до 615.6 транзакций в секунду (-2.2%). Лично я бы списал эту разницу либо на неудачные настройки для дисковой подсистемы кластера (параметр `effective_io_concurrency`), либо на погрешность измерения.
